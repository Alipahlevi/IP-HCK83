...
describe('Error handling for generateRecipeWithAI', () => {
  it('should handle invalid JSON response from AI', async () => {
    ...
    expect(response.body.message).toBe('Failed to generate recipe');
  });

  it('should handle AI response missing required fields', async () => {
    // Mock AI to return incomplete data
    const { GoogleGenerativeAI } = require('@google/generative-ai');
    const mockGenAI = new GoogleGenerativeAI();
    const mockModel = mockGenAI.getGenerativeModel();
    ...
    mockModel.generateContent.mockResolvedValueOnce({
      response: {
        text: () => JSON.stringify({ title: 'Test' }) // Missing ingredients and instructions
      }
    });
